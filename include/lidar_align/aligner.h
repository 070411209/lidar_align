...
public:
    struct Config{
        bool local = false;
        // (x,y,z,rx,ry,rz)
        std::vector<double> inital_guess{-3.1929,  0.0869, -0.9017, -0.018057, 0.00613, -1.56328, 0.0};
...
private:
    void optimize(const std::vector<double> &lb, const std::vector<double> &ub, OptData *opt_data, std::vector<double> *x);
    std::string generateCalibrationString(const Transform &T, const double time_offset);
    ...
    float lidarOdomKNNError(const Pointcloud &base_pointcloud, const Pointcloud &combined_pointcloud) const;
