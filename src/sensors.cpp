...
OdomTformData::OdomTformData(Timestamp timestamp_us, Transform T_o0_ot) : timestamp_us_(timestamp_us), T_o0_ot_(T_o0_ot) {}
...
void Odom::addTransformData(const Timestamp &timestamp_us, const Transform &T){
    data_.emplace_back(timestamp_us, T);        //~ use default construct function `OdomTformData(time, T) to create and push.
}


//~ get transform using interp.
Transform Odom::getOdomTransform(const Timestamp timestamp_us, const size_t start_idx, size_t *match_idx) const{
    size_t idx = start_idx;
    while ((idx < (data_.size() - 1)) && (timestamp_us > data_[idx].getTimestamp()))
        ++idx;
    if (idx > 0)
        --idx;
    ...
    // interpolate
    double t_diff_ratio = static_cast<double>(timestamp_us - data_[idx].getTimestamp()) /
                            static_cast<double>(data_[idx + 1].getTimestamp() - data_[idx].getTimestamp());

    Transform::Vector6 diff_vector = (data_[idx].getTransform().inverse() * data_[idx + 1].getTransform()).log();
    Transform out = data_[idx].getTransform() * Transform::exp(t_diff_ratio * diff_vector);
...
Scan::Config Scan::getConfig(ros::NodeHandle *nh){
    Scan::Config config;
    nh->param("min_point_distance", config.min_point_distance, config.min_point_distance);
    nh->param("max_point_distance", config.max_point_distance, config.max_point_distance);
    nh->param("keep_points_ratio", config.keep_points_ratio, config.keep_points_ratio);
    nh->param("min_return_intensity", config.min_return_intensity, config.min_return_intensity);
    nh->param("estimate_point_times", config.estimate_point_times, config.estimate_point_times);
    nh->param("clockwise_lidar", config.clockwise_lidar, config.clockwise_lidar);
    nh->param("motion_compensation", config.motion_compensation, config.motion_compensation);
    nh->param("lidar_rpm", config.lidar_rpm, config.lidar_rpm);
    return config;
}

void Scan::setOdomTransform(const Odom &odom, const double time_offset, const size_t start_idx, size_t *match_idx){
    T_o0_ot_.clear();
    size_t i = 0;
    for (Point point : raw_points_){
        ...
        Timestamp point_ts_us = timestamp_us_ + static_cast<Timestamp>(1000000.0 * time_offset) + static_cast<Timestamp>(point.intensity);
...
void Scan::getTimeAlignedPointcloud(const Transform &T_o_l, Pointcloud *pointcloud) const{  //~ undistort points in one scan.
...
void Lidar::addPointcloud(const LoaderPointcloud &pointcloud, const Scan::Config &config){
    scans_.emplace_back(pointcloud, config);
}

void Lidar::getCombinedPointcloud(Pointcloud *pointcloud) const {
    for (const Scan &scan : scans_)
        scan.getTimeAlignedPointcloud(getOdomLidarTransform(), pointcloud); //~ convert to Odom-coordinate (by estimated T_o_l_)
}
...
void Lidar::saveCombinedPointcloud(const std::string &file_path) const
{
    ...
    pcl::io::savePCDFileASCII("/home/one/test_ws/src/lidar_align/results/lidar_combined.pcd", combined);
}
